...
#' @export
#' @examples predict_state_one_mna("parameter", "Green Control", as.matrix(out$params), as.matrix(out$predict), data, 500, sample.int(nrow(as.matrix(out$params)), 500, replace = TRUE))


predict_state_one_mna <- function(type, site, params, ic, data, Nmc, draw){
  ...
  if("parameter" %in% type){
    ...
    beta.21 <- params[draw, "beta.21"]
    beta.32 <- params[draw, "beta.32"]
  } else {
    ...
    beta.21 <- rep(param.mean["beta.21"], Nmc)
    beta.32 <- rep(param.mean["beta.32"], Nmc)
  }
  ...
  for(m in 1:Nmc){
    ...
    A[2,1,] <- logit(grow.ln.mu[m] + beta.21[m]*mna)
    A[2,2,] <- logit(phi.n.mu[m])
    A[3,2,] <- logit(grow.na.mu[m] + beta.32[m]*mna)
